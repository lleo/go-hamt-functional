...
func h60ToString(h60 uint64) string {
	return hashPathString(h60, MaxDepth)
}

func StringToH60(s string) uint64 {
...
// h60ToBitStr is for printf debugging use
func h60ToBitStr(h60 uint64) string {
...
func indexMask(depth uint) uint64 {
	return uint64((1<<Nbits)-1) << (depth * Nbits)
}
...
func buildHashPath(hashPath uint64, idx, depth uint) uint64 {
	var mask uint64 = (1 << ((depth - 1) * Nbits)) - 1
	hashPath = hashPath & mask

	return hashPath | uint64(idx<<((depth-1)*Nbits))
}
